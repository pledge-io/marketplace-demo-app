document.addEventListener('DOMContentLoaded', function () {
  ...
  if (checkoutQuantityInput) {
    ...
    document.getElementById('checkout-quantity').addEventListener('keyup', calculateOrder);
  }
  ...
  if (simulatePaymentButton) {
    ...
    simulatePaymentButton.addEventListener('click', simulateSuccessfulPayment);
  }
});
...
function calculateOrder(event) {
  ...
  document.getElementById('checkout-subtotal').innerHTML = `${currency} ${subtotal.toFixed(2)}`;

  document.getElementById('checkout-service-fee').innerHTML = `${currency} ${serviceFee.toFixed(2)}`;

  document.getElementById('checkout-total').innerHTML = `${currency} ${total.toFixed(2)}`;
}
